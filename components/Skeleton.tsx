...
const SkeletonPulse: React.FC<{ className: string }> = ({ className }) => (
  <motion.div
    ...
    className={`${className} bg-slate-200 border border-slate-300`}
  />
);
  ...
  return (
    <div className="space-y-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="p-4 bg-white border-2 border-black newspaper-shadow-sm">
...
export const LiveCardSkeleton: React.FC<SkeletonProps> = ({ count = 4 }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex flex-col md:flex-row h-auto md:h-48 bg-white border-2 border-black newspaper-shadow-sm">
...
export const CircleCardSkeleton: React.FC<SkeletonProps> = ({ count = 8 }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="h-auto pb-4 overflow-hidden bg-white border-2 border-black newspaper-shadow-sm">
          <SkeletonPulse className="h-24 w-full" />
          <div className="px-4 relative -mt-8">
             <SkeletonPulse className="w-16 h-16 mb-3 border-4 border-white" />
