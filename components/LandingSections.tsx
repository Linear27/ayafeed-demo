...
import { getCurrentDateKey, getLiveStatus, toDateKey, toMonthDay } from '../utils/date';
...
export const HeroCarousel: React.FC<{
  ...
  const displayDate = toDateKey(featuredEvent.startAt);
  const displayRegion =
    featuredEvent.marketRegion === 'JAPAN'
      ? 'JP'
      : featuredEvent.marketRegion === 'CN_MAINLAND'
        ? 'CN'
        : 'OVERSEA';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        <div className="lg:col-span-8 flex flex-col">
          <Link
            ...
            className="relative block w-full h-[280px] sm:h-[360px] group"
          >
            <div className="w-full h-full relative overflow-hidden flex items-center justify-center border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] bg-[#E5E5E5] p-2">
              ...
              <div className="absolute -bottom-4 -left-4 z-20">
                <div className="stamp text-lg bg-white/90 backdrop-blur-sm shadow-sm">{displayRegion}</div>
              </div>
            </div>
          </Link>
          ...
          <div className="mt-3 flex items-center justify-between gap-4 text-xs font-bold font-mono uppercase tracking-widest text-slate-500">
            <div className="flex items-center gap-1">
              <User size={12} /> ORG: {featuredEvent.id}
            </div>
            <div className="text-right flex items-center gap-1">
              <Globe size={12} /> {displayRegion} 分社
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col">
            <Link
              to="/events/$eventId"
              params={{ eventId: featuredEvent.id }}
              className="text-2xl sm:text-3xl lg:text-4xl font-black leading-[1.1] mb-4 hover:text-red-600 transition-colors font-header line-clamp-2"
            >
              {featuredEvent.title}
            </Link>
            ...
            <div className="grid grid-cols-2 gap-px border-2 mb-4 bg-black border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              ...
              <InfoCell
                icon={MapPin}
                label="地点"
                value={featuredEvent.location?.name || '未知'}
                className="bg-white"
              />
            </div>
        ...
        {subEvents.length > 0 && (
          <div className="lg:col-span-4 flex flex-col gap-4 lg:border-l-2 border-black lg:pl-6">
            ...
            {subEvents.map((subEvent) => {
              const subDate = toDateKey(subEvent.startAt);
              const subRegion =
                subEvent.marketRegion === 'JAPAN'
                  ? 'JP'
                  : subEvent.marketRegion === 'CN_MAINLAND'
                    ? 'CN'
                    : 'OVERSEA';
              ...
              return (
                <Link
                  key={subEvent.id}
                  to="/events/$eventId"
                  params={{ eventId: subEvent.id }}
                  className="group flex gap-3 pb-4 border-b-2 border-dashed border-slate-300 last:border-0 last:pb-0"
                >
                  <div className="w-20 h-24 shrink-0 border-2 border-black bg-slate-100 flex items-center justify-center p-1 relative shadow-sm group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
                    <img
                      src={subEvent.poster?.urls.original || null}
...
const ScoopItem: React.FC<{ event: PublicEventListItem; isToday: boolean }> = ({ event, isToday }) => {
  const displayDate = toDateKey(event.startAt);
  const displayRegion =
    event.marketRegion === 'JAPAN' ? 'JP' : event.marketRegion === 'CN_MAINLAND' ? 'CN' : 'OVERSEA';
    ...
    <Link to="/events/$eventId" params={{ eventId: event.id }} className="group block">
      <div className="flex gap-4 items-start relative min-h-[160px]">
        <div className="w-32 sm:w-40 shrink-0">
          <div
            className={`border-2 p-1 bg-white shadow-sm transition-all group-hover:shadow-md h-[140px] ${
              isToday ? 'border-red-600' : 'border-black'
            }`}
          >
            <div className="overflow-hidden relative h-full flex items-center justify-center bg-slate-100">
              <img
                src={event.poster?.urls.original || null}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                alt={event.title}
              />
        ...
        <div className="flex-1 min-w-0 flex flex-col py-0.5">
          <div className="flex justify-between items-center mb-1">
            <span
              className={`text-xs sm:text-[12px] font-black uppercase tracking-widest ${
          ...
          <p className="text-sm mb-3 line-clamp-2 leading-relaxed text-slate-700 font-serif">{event.summary}</p>
    ...
    <div className="lg:col-span-8 flex flex-col">
      ...
      {hasEvents ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
            {events.map((event, idx) => (
              <ScoopItem key={`${event.id}-${idx}`} event={event} isToday={toDateKey(event.startAt) === todayStr} />
            ))}
          </div>
          <div className="mt-8">
            <Link
        ...
        <div className="border-2 border-dashed border-slate-300 bg-white/60 p-8 sm:p-10 text-center">
          <div className="text-xs font-black uppercase tracking-[0.22em] text-slate-500">NO UPCOMING</div>
          <h3 className="mt-3 text-xl sm:text-2xl font-black font-header text-slate-900">暂无可展示的速报条目</h3>
      ...
      {hasLives ? (
        <>
          <div className="space-y-4">
            {lives.map((live) => {
              const timeZone = live.location?.timeZone || 'Asia/Tokyo';
              const todayKey = getCurrentDateKey(timeZone);
              const status = getLiveStatus(live.startAt, todayKey);
              const { month, day } = toMonthDay(live.startAt);
              ...
              return (
                <Link
                  ...
                  className="group cursor-pointer flex gap-3 pb-4 border-b border-dashed border-slate-300 last:border-0 last:pb-0"
                >
                  <div className="w-14 h-16 shrink-0 border-2 border-black bg-white flex flex-col items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] group-hover:-translate-y-0.5 transition-all">
                    <span className="text-[10px] font-black uppercase text-slate-500 border-b-2 border-slate-200 w-full text-center pb-0.5 mb-0.5">
                      {month}月
                    </span>
        ...
        <div className="border-2 border-dashed border-slate-300 bg-white/60 p-8 text-center">
          <div className="text-xs font-black uppercase tracking-[0.22em] text-slate-500">NO UPCOMING</div>
          <h3 className="mt-3 text-lg sm:text-xl font-black font-header text-slate-900">暂无舞台排程</h3>
          <p className="mt-2 text-sm text-slate-600 font-serif italic max-w-[60ch] mx-auto">
