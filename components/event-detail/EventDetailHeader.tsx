...
const EventDetailHeader: React.FC<EventDetailHeaderProps> = ({ event, city, circleCount, onBack, onSelectCirclesTab }) => {
    ...
    <div className="bg-white border-b-2 border-black relative z-10 pb-2">
      <div className="max-w-7xl mx-auto">
        <div className="px-4 py-3 flex items-center justify-between">
          <button onClick={onBack} className="flex items-center text-slate-900 hover:text-red-600 text-[10px] sm:text-sm font-black font-mono uppercase tracking-tighter">
            <ChevronLeft size={18} className="mr-1" /> 返回索引
          </button>
          <div className="text-[9px] sm:text-[10px] font-mono text-slate-400">报告编号: {event.slug.toUpperCase()}</div>
        </div>

        <div className="px-4 pb-6 sm:pb-8 pt-2">
          <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
            {/* Poster Section */}
            {event.poster?.urls.original && (
              <div className={`w-full ${isLandscape ? 'md:w-96' : 'md:w-80'} shrink-0 relative flex justify-center`}>
                ...
                <div className={`${isLandscape ? 'aspect-video' : 'aspect-[3/4]'} w-full max-w-[320px] sm:max-w-none border-2 border-black bg-white p-2 shadow-lg relative z-10 transform -rotate-1 transition-transform hover:rotate-0 duration-500`}>
                  <div className="w-full h-full bg-slate-200 overflow-hidden relative">
                    <img src={event.poster?.urls.original || null} className="w-full h-full object-cover" alt="Event Poster" />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                <span className="bg-red-600 text-white px-2 py-0.5 text-[10px] font-black uppercase tracking-wider">专题报导</span>
                {event.genres?.slice(0, 2).map(g => (
                  <span key={g} className="px-2 py-0.5 text-[10px] font-black border border-black uppercase tracking-tighter bg-white">{g}</span>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 font-header">
                <div className="flex items-start gap-4 p-4 border-2 border-black bg-white newspaper-shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xl sm:text-2xl font-black leading-none">{event.startAt}</div>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-4 border-2 border-black bg-white newspaper-shadow-sm">
                    ...
                    <div className="text-lg sm:text-xl font-black leading-tight flex flex-wrap items-center">
                      {city && (
                        <div className="mr-3 px-2 py-1 flex items-center bg-white border-2 border-slate-900 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                          <span className="text-base font-black tracking-[0.15em]">{city}</span>
                        </div>
                      )}
                    </div>
                  </div>
